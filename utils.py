...
def use_heatmap(image, box_centers):
    ...
    img = hm.heatmap(box_centers, dotsize=10, size=(image.shape[1], image.shape[0]), opacity=40, area=((0, 0), (image.shape[1], image.shape[0])))
    img.save('hm.png')
    #return np.array(img)
...
def expand_pixel(ratio, pos, w_min, w_max, h_min, h_max):
    #ration is odd number
    pad = (ratio - 1.0) // 2.0
    L,R,T,D = max(w_min,pos[0]-pad), min(w_max,pos[0]+pad), max(h_min,pos[1]-pad), min(h_max,pos[1]+pad)
    return [(int(L),int(T)),(int(R),int(D))]

def view_angle_pressure(center_pos,center_dir,center_vel,pos,vs,ds):
	angle = 60.0
	...
	for p,vec in zip(pos,Vectors):
		if distance(p,center_pos) <= 5.0:
			continue
		if angle_between(vec,center_vector) <= angle:
